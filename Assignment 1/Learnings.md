...
Most files need to have this to print errors:
```C
...
every element is handled by exactly one thread, and
we can use tid as the array index: C[tid] = A[tid] + B[tid].
In 1D:
...
And so on.
So the unique global index for this thread is:
